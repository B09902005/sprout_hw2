...
MainGame::MainGame() {
	...
	this->bullet2_img = ImageProcess::load_bitmap_at_size("./image/bullet4.png", scale, scale * 2);
	if (!this->bullet2_img)
		LOG::game_abort("failed to load bullet2 image");
    // load another bullet image
    this->bullet3_img = ImageProcess::load_bitmap_at_size("./image/bullet.png", scale, scale * 2);
    if (!this->bullet3_img)
...
if (this->P3->hp > 0){
    ...
    model.update(object_list, board, this->P2);
...
if (this->P2->hp > 0){
    ...
    model.update(object_list, board, this->P4);
if (this->P4->hp > 0){
    if(board[ALLEGRO_KEY_D]){
